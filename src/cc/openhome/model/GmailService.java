...
public class GmailService implements EmailService {
    ...
    public void validationLink(Account acct) {
        try {
            String link = String.format(
                "http://localhost:8080/Final_Project/verify?email=%s&token=%s", 
            ...
            javax.mail.Message message = createMessage(
                    mailUser, acct.getEmail(), "圖片上傳網站註冊結果", html);
    ...
    public void failedRegistration(String acctName, String acctEmail) {
        try {
            javax.mail.Message message = createMessage(mailUser, acctEmail, "圖片上傳網站註冊結果", 
    ...
    public void passwordResetLink(Account acct) {
        try {
            String link = String.format(
                "http://localhost:8080/Final_Project/reset_password?name=%s&email=%s&token=%s", 
            ...
            javax.mail.Message message = createMessage(
                    mailUser, acct.getEmail(), "圖片上傳網站重設密碼", html);
            Transport.send(message);
        } catch (MessagingException | IOException e) {
            throw new RuntimeException(e);
        }
